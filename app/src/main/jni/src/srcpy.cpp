#include "vendor/enc/oxorany.h"

#define O(x) oxorany(x)
...
if not os.path.exists(ENTITY_FIFO_PATH):
    os.mkfifo(ENTITY_FIFO_PATH)
...
def EspUpdate(*_, **__):
    ...
    if not localEntity or not hasattr(localEntity, 'model'):
        return
    ...
    for entityId, entity in inst.entities.copy().items():
        if not isinstance(entity, CombatAvatar):
            continue
        ...
        if entity == localEntity:
            continue
        if is_same_team(entity, localEntity):
            continue
        ...
        if not head_screen or head_screen.z <= 0:
            continue
...
StoryTick._instance.Add(EspUpdate, 120)
...
StoryTick._instance.Add(AIMUpdate, 120)
...
def GetTargetEntity(maxDist=None):
    ...
    targetEntity = None
    nearestDistance = maxDist or float('inf')
    ...
    for entityId, entity in Space._instance.entities.copy().items():
        ...
        entityPosx = WorldToScreenPoint(head)
        if not entityPosx or entityPosx.z <= 0.0: continue
...
LOG_PATH = "/sdcard/player_bug_damage_log.txt"

def save_log(*msg):
    try:
        with open(LOG_PATH, "a", encoding="utf-8") as f:
            f.write(f"[{time.strftime('%H:%M:%S')}] " + " ".join(map(str, msg)) + "\n")
    except:
        pass
...
def CallServerGameLogic(self, method, *args, **kwargs):
    if sync_attrs.get('bBulletTrack') and method == 'BatchDealSpellResult' and Space._instance.entities:
        for hit in args[0].copy():
            if 'caster' not in hit:
                save_log("BUG.NO_CASTER", hit)
                continue
            
            localEntity = genv.avatar.combat_avatar
            if not localEntity:
                save_log("BUG.NO_LOCAL_ENTITY")
                continue
            
            if localEntity.id != hit['caster']:
                save_log("BUG.CASTER_NOT_LOCAL", hit.get('caster'), localEntity.id)
                continue
            
            probability = sync_attrs.get('fBulletTrack_Probability', 70) / 100
            
            if 'damage_result' in hit:
                if not trueChance(probability):
                    save_log("BUG.PROBABILITY_FAIL")
                    continue
                
                for _, hitData in hit['damage_result'].items():
                    if hitData.get('hit_part', 'Head') == 'Head':
                        save_log("BUG.ALREADY_HEAD", hitData.get('target_id'))
                        continue
                    
                    if not hitData.get('weapon_guid'):
                        save_log("BUG.NO_WEAPON_GUID_IN_HITDATA", hitData)
                    
                    weapon_guid = hitData.get('weapon_guid') or hit.get('weapon_guid')
                    if not weapon_guid:
                        save_log("BUG.NO_WEAPON_GUID_FINAL")
                        continue
                    
                    curWeapon = localEntity.GetWeaponByGuid(weapon_guid)
                    if not curWeapon:
                        save_log("BUG.WEAPON_NOT_FOUND", weapon_guid)
                        continue
                    ...
                    if not hitEntity:
                        save_log("BUG.TARGET_ENTITY_NOT_FOUND", _)
                        continue
                    
                    if not isinstance(hitEntity, CombatAvatar):
                        save_log("BUG.TARGET_NOT_COMBAT_AVATAR", type(hitEntity))
                        continue
                    
                    if not hitEntity.is_alive:
                        save_log("BUG.TARGET_NOT_ALIVE", hitEntity.id)
                        continue
                    
                    if sync_attrs.get('bBulletTrack_IgnoreBots') and hitEntity.IsRobotCombatAvatar:
                        save_log("BUG.TARGET_IS_BOT", hitEntity.id)
                        continue
                ...
                if localEntity.is_ads:
                    probability += 0.10
                
                if not trueChance(probability):
                    save_log("BUG.FORCE_PROBABILITY_FAIL")
                    continue
                
                curWeapon = localEntity.GetCurWeapon()
                if not curWeapon:
                    save_log("BUG.NO_CURRENT_WEAPON")
                    continue
                ...
                if not targetEntity:
                    save_log("BUG.NO_TARGET_ENTITY")
                    continue
                
                if sync_attrs.get('bBulletTrack_IgnoreBots') and targetEntity.IsRobotCombatAvatar:
                    save_log("BUG.TARGET_IS_BOT_FORCE", targetEntity.id)
                    continue
                
                if sync_attrs.get('bBulletTrack_IgnoreKnocked') and targetEntity.is_dying_state:
                    save_log("BUG.TARGET_KNOCKED", targetEntity.id)
                    continue
                ...
                if not raycast:
                    save_log("BUG.RAYCAST_FAIL_FORCE", targetEntity.id)
                    continue
                
                damage = float(curWeaponProto[bone['dmg_name']])
                damage = dropDamage(curWeaponProto, damage, distance)
                if not damage:
                    save_log("BUG.DAMAGE_ZERO_FORCE", targetEntity.id, distance)
                    continue
...
def Create(wid, *args, **kwargs):
    ...
    if sync_attrs.get('bSkinHack'):
        selected_skin_id = int(sync_attrs.get('iSelectedSkinID', 0))

        if selected_skin_id > 0:
            # Gunakan skin yang dipilih secara manual/spesifik dari menu
            args[4] = selected_skin_id
        else:
            # Mode Random atau Default jika iSelectedSkinID == 0
            if wid in WEP:
                wids = WEP[wid][1:]
